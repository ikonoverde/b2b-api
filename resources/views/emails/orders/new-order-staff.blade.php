...
<body style="margin:0; padding:0; background-color:#dce7e8;">
    <div style="display:none; max-height:0; overflow:hidden; opacity:0; font-size:1px; line-height:1px; color:#dce7e8;">
        Nuevo pedido #{{ $order->id }} de {{ $order->user->name }} — ${{ number_format((float) $order->total_amount, 2) }}.
    </div>
                ...
                <table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px; max-width:600px; background-color:#ebf6f7; border:1px solid #dce7e8;">
                    ...
                    <tr>
                        <td class="pad" style="padding:40px 40px 8px 40px;">
                            <table role="presentation" cellpadding="0" cellspacing="0">
                                <tr>
                                    ...
                                    <td valign="middle" style="padding-left:16px; font-family:{{ $sans }}; font-size:12px; font-weight:500; letter-spacing:0.06em; color:#006871; text-transform:uppercase;">
                                        Pago recibido &middot; {{ $itemCount }} {{ $itemCount === 1 ? 'unidad' : 'unidades' }}
                                    </td>
                                </tr>
                            </table>
                            <h1 style="margin:24px 0 0 0; font-family:{{ $serif }}; font-size:34px; line-height:1.1; font-weight:400; letter-spacing:-0.01em; color:#0f1d1e;">
                                Nuevo pedido <span style="font-family:{{ $mono }}; font-size:28px;">#{{ $order->id }}</span>
                            </h1>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td class="pad" style="padding:16px 40px 8px 40px; font-family:{{ $sans }}; font-size:15px; line-height:1.55; color:#3a4a4b;">
                            Se ha confirmado el pago de un pedido y está listo para preparación.
                        </td>
                    </tr>

                    {{-- Customer --}}
                    <tr>
                        <td class="pad" style="padding:24px 40px 0 40px; font-family:{{ $sans }}; font-size:11px; font-weight:500; letter-spacing:0.06em; color:#586767; text-transform:uppercase;">
                            Cliente
                        </td>
                    </tr>
                    <tr>
                                ...
                                <tr>
                                    <td style="padding:16px 20px;">
                                        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-family:{{ $sans }}; font-size:13px;">
                                            <tr>
                                                <td style="padding:5px 0; color:#586767;">Nombre</td>
                                                <td align="right" style="padding:5px 0; font-size:13px; font-weight:500; color:#0f1d1e;">{{ $order->user->name }}</td>
                                            </tr>
                                            <tr>
                                                <td style="padding:5px 0; color:#586767; border-top:1px solid #e4eeef;">Correo</td>
                                                <td align="right" style="padding:5px 0; font-family:{{ $mono }}; font-size:12px; color:#0f1d1e; border-top:1px solid #e4eeef; word-break:break-all;">{{ $order->user->email }}</td>
                                            </tr>
                                            @if($order->user->phone)
                                            <tr>
                                                <td style="padding:5px 0; color:#586767; border-top:1px solid #e4eeef;">Teléfono</td>
                                                <td align="right" style="padding:5px 0; font-family:{{ $mono }}; font-size:12px; color:#0f1d1e; border-top:1px solid #e4eeef;">{{ $order->user->phone }}</td>
                                            </tr>
                                            @endif
                                            @if($order->user->rfc)
                                            <tr>
                                                <td style="padding:5px 0; color:#586767; border-top:1px solid #e4eeef;">RFC</td>
                                                <td align="right" style="padding:5px 0; font-family:{{ $mono }}; font-size:12px; color:#0f1d1e; border-top:1px solid #e4eeef;">{{ $order->user->rfc }}</td>
                                            </tr>
                                            @endif
                                        </table>
                                    </td>
                    ...
                    <tr>
                        <td class="pad" style="padding:24px 40px 0 40px;">
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-family:{{ $sans }}; font-size:13px;">
                                <tr>
                                    <td style="padding:6px 0; color:#586767;">Recibido</td>
                                    <td align="right" class="num" style="padding:6px 0; font-family:{{ $mono }}; font-size:13px; color:#0f1d1e;">{{ $order->created_at->format('d/m/Y H:i') }}</td>
                                </tr>
                                <tr>
                                    ...
                                    <td align="right" style="padding:6px 0; border-top:1px solid #e4eeef;">
                                        <span style="display:inline-block; padding:3px 10px; border-radius:11px; background-color:#dcefef; font-family:{{ $sans }}; font-size:12px; font-weight:600; color:#005159;">
                                            <span style="color:#006871;">&#9679;</span>&nbsp;{{ $paymentStatus }}
                                        </span>
                                    </td>
                                </tr>
                    ...
                    {{-- Footer --}}
                    <tr>
                        <td class="pad" style="padding:32px 40px 32px 40px; border-top:1px solid #dce7e8;">
                            <div style="font-family:{{ $sans }}; font-size:11px; line-height:1.5; color:#8a9696;">
                                Notificación interna de Ikono<span style="color:#1a9d11;">verde</span>. No reenviar al cliente.
                            </div>
                        </td>
                    </tr>
