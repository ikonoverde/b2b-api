...
## Admin login gotchas
- Admin routes return **404 for guests** (admin is hidden), so `goto /admin` unauthenticated shows the `Error` page — go to `/admin/login` first. The dashboard lives at `/admin` (there is no `/admin/dashboard` route).
