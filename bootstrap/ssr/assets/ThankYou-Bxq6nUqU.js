...
import { usePoll, Link } from "@inertiajs/react";
...
import { d as trackGoogleAnalyticsPurchase, e as trackMetaPurchase, M as META_PIXEL_CURRENCY } from "./analytics-BNINfFmc.js";
import "lucide-react";
...
function purchaseStorageKey(platform, eventId) {
  return `${platform}_purchase_tracked:${eventId}`;
}
function hasTrackedPurchase(platform, eventId) {
  try {
...
function rememberTrackedPurchase(platform, eventId) {
  try {
...
function ThankYou({ order }) {
  const { start: startPaymentStatusPolling, stop: stopPaymentStatusPolling } = usePoll(2e3, {
    only: ["order"]
  }, {
    autoStart: false
  });
